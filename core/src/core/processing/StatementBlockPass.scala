...
class StatementBlockPass
  ...
  override def visit(stmt: IfStatement): Int = {
    ...
    if (!stmt.elseBody.isDefined&&(!stmt.elseifBodies.isDefined)) {
      stmt.update(thenBody = thenBody)
      StatementVisitor.Continue

    }else if(!stmt.elseBody.isDefined){
      val elseifBodies = stmt.elseifBodies.get match {
        case s: List[Statement] => Some(s)
        case other             => Some( other)//Some( StatementBlock(List(other)))
      }
      stmt.update(thenBody = thenBody, elseifBodies = elseifBodies)
      StatementVisitor.Continue
    }else {
      val elseifBodies = stmt.elseifBodies.get match {
        case s: List[Statement] => Some(s)
      ...
      stmt.update(thenBody = thenBody,elseifBodies= elseifBodies, elseBody = elseBody)
