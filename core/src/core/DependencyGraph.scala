...
object DependencyGraph {
  ...
  def printRawGraph(stmts: List[AssignmentStatement], graph: Graph[AssignmentStatement, DistVecEdge]) = {
  ...
  def printReducedGraph(stmts: List[AssignmentStatement], graph: Graph[Set[AssignmentStatement], SCCDistVecEdge]) = {
    def getHash(stmt: AssignmentStatement) = {
      stmt.toString.substring(stmt.toString.indexOf("@"))
    }
  ...
  def topoSort(stmts: List[AssignmentStatement], graph: Graph[Set[AssignmentStatement], SCCDistVecEdge]) = {
    val order = stmts.zipWithIndex.toMap
    val filterNodes = graph.nodes.filter(n => n.inDegree == 0).map(n => n.value)
    val noIncomingNodes = ListBuffer.empty[Set[AssignmentStatement]] ++ filterNodes.toList.sortWith((n1, n2) => order(n1.value.head) < order(n2.value.head))
    ...
    while (noIncomingNodes.size > 0) {
      ...
      for (e <- dstEdges) {
        graph -= e
        // get(e.to) does not work
        val m = graph.get(e._2)
...
//          println(PrettyPrinter.pretty(s1) + "," + PrettyPrinter.pretty(s2))
//          println(PrettyPrinter.pretty(s1.writeRef) + "," + PrettyPrinter.pretty(s2.writeRef) + ":")
//          println(s2.writeRef == s1.writeRef)
//          println(reads)
//          val r = s2.readRefs
//          println(dists)
...
//        }
  ...
  def reduceSCC(graph: Graph[AssignmentStatement, DistVecEdge]) = {
    
    var index = 0
    val nodeIndex = mutable.HashMap.empty[graph.NodeT, Int] ++ graph.nodes.map(n => (n, -1))
    val nodeLowLink = mutable.HashMap.empty[graph.NodeT, Int] ++ graph.nodes.map(n => (n, -1))
    ...
    for (fromSCC <- SCCs) {
      ...
      // contains(e.to) does not work
      val dstEdges = srcEdges.filter(e => !fromSCC.contains(e._2))
      // group edges by destination scc containing the to-node
      val edgesGroup = dstEdges.groupBy(e => findSCC(graph get e._2))
